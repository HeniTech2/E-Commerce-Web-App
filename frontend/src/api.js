...
// Keys whose values are stored as images (uploaded via multipart)
...
export const getAdminSettings = async () => {
  ...
  (data.content || []).forEach((item) => {
    if (item.key.startsWith("admin_")) {
      const k = item.key.replace("admin_", "");
      // Image keys store URL in item.image; text keys store value in item.body
      settings[k] = IMAGE_KEYS.has(k)
        ? item.image || item.body || ""
        : item.body || item.title || item.image || "";
    }
  });
  return { success: true, settings };
};
...
export const saveAdminSetting = async (key, value) => {
  ...
  // Image keys are handled via multipart upload in Settings.jsx uploadFile()
  // — skip here to avoid overwriting the image URL with a text body save
