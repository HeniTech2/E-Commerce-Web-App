...
const ProductImage = ({ src, alt, className }) => {
  ...
  if (!resolved || failed) {
    return (
      <div
        className={`${className} bg-surface border border-border rounded-lg flex items-center justify-center`}
      >
...
const AdminProducts = () => {
  ...
  const inputClass =
    "w-full border border-border bg-paper2 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary";
  ...
  const handleVideoChange = (file) => {
    ...
    if (file.size > 50 * 1024 * 1024) {
      toast.warning(
        "Video is large (>50 MB). Upload may take a while — please be patient."
      );
    }
    setVideo(file);
  ...
  const save = async (e) => {
    e.preventDefault();
    setSaving(true);
    setSaveStatus(video ? "uploading" : "saving");
  ...
  const existingImages = editTarget ? (editTarget.image || []).slice(0, 4) : [];
  const existingVideo = editTarget?.video || null;

  // Label shown inside the save button
  const savingLabel =
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          onClick={openAdd}
          className="flex items-center gap-2 bg-primary text-white rounded-xl px-5 py-3 text-sm font-semibold hover:bg-primaryDark transition-colors shadow-card"
        >
      ...
      {mode && (
        <div className="fixed inset-0 bg-ink/40 flex items-center justify-center p-4 z-50">
          <form
            onSubmit={save}
            className="bg-white w-full max-w-md rounded-2xl shadow-dropdown p-6 overflow-y-auto max-h-[90vh]"
          >
            ...
            <div className="space-y-3">
              ...
              <div>
                <p className="text-xs text-stone mb-1">
                  {mode === "edit"
                    ? "New images (leave empty to keep current)"
                ...
                {mode === "edit" && existingImages.length > 0 && (
                  <div className="flex gap-2 mb-2">
                    {existingImages.map((img, i) => (
                      <div
                        key={i}
                        className="w-14 h-16 rounded-lg overflow-hidden border border-border"
                      >
                        <ProductImage
                          src={img}
                          alt=""
                    ...
                    <p className="text-xs text-stone self-center ml-1">
                      Current images
                    </p>
                  </div>
                )}
                <div className="grid grid-cols-4 gap-2">
                  {["image1", "image2", "image3", "image4"].map((key) => (
                    <label
                      key={key}
                      className="aspect-square border-2 border-dashed border-border rounded-xl flex items-center justify-center cursor-pointer hover:border-primary/60 transition-colors overflow-hidden relative group"
                    >
                      {previews[key] ? (
                        <>
                          <img
              ...
              <div>
                <p className="text-xs text-stone mb-1 flex items-center gap-1">
                  <HiVideoCamera size={13} /> Product video (optional — mp4,
                  mov, webm)
                </p>
                  ...
                  !videoPreview && (
                    <div className="relative rounded-xl overflow-hidden border border-border mb-2">
                      ...
                      <div className="absolute top-2 right-2 flex gap-1 items-center">
                        <span className="bg-black/60 text-white text-[10px] px-2 py-0.5 rounded-full font-semibold">
                          Current video
                        </span>
                        <button
                          type="button"
                          onClick={removeVideoSelection}
                          className="bg-danger text-white rounded-full p-1 hover:bg-red-700 transition-colors"
                        >
                          <HiXMark size={12} />
                        </button>
                      </div>
                ...
                {!videoPreview &&
                  !(mode === "edit" && existingVideo && !clearVideo) && (
                ...
                {video && (
                  <p className="text-[10px] text-stone mt-1">
                    ⏳ Large videos may take 15–60 seconds to upload to
                    Cloudinary. Please wait after clicking save.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-5">
              <button
                ...
                disabled={saving}
                className="border border-border rounded-xl px-5 py-2.5 text-sm font-semibold hover:bg-surface transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={saving}
                className="flex-1 bg-primary text-white rounded-xl px-5 py-2.5 text-sm font-semibold hover:bg-primaryDark transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
              >
                {saving && (
