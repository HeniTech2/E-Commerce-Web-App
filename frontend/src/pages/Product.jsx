...
const Product = () => {
  ...
  if (productsLoading) return <div className="max-w-7xl mx-auto px-5 py-24 text-center text-stone">Loading…</div>;

  if (!product) {
    return <div className="max-w-7xl mx-auto px-5 py-24 text-center font-display text-2xl font-bold">Product not found.</div>;
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 md:px-8 py-12">
      <div className="text-xs text-stone mb-8">
        <Link to="/" className="hover:text-primary">Home</Link> / <Link to="/shop" className="hover:text-primary">Shop</Link> / {product.name}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-12">
        ...
        <div>
          <span className="inline-block bg-primaryLight text-primary text-xs font-semibold px-3 py-1.5 rounded-full uppercase tracking-wide">{product.category}</span>
          <h1 className="font-display text-3xl md:text-4xl font-bold mt-3">{product.name}</h1>

          <div className="flex items-center gap-4 mt-3">
            <div className={`flex items-center gap-1.5 text-sm font-medium ${stock.text}`}>
          ...
          {product.sizes?.length > 0 && (
            <div className="mt-5">
              <p className="text-sm font-semibold mb-2">
                Sizes
                {sizeError && <span className="ml-2 text-danger font-normal text-xs">— Please select a size</span>}
              </p>
          ...
          <div className="mt-8 flex items-center gap-4">
            <div className="flex items-center border border-border rounded-xl overflow-hidden">
              <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="px-4 py-3 hover:bg-surface transition-colors font-semibold">−</button>
              <span className="px-4 font-semibold">{qty}</span>
              <button onClick={() => setQty((q) => q + 1)} className="px-4 py-3 hover:bg-surface transition-colors font-semibold">+</button>
            </div>
            <button
              onClick={handleAdd}
              disabled={outOfStock}
              className={`flex-1 px-8 py-3.5 rounded-xl font-semibold text-sm transition-colors flex items-center justify-center gap-2 ${
                outOfStock ? "bg-surface text-stoneLight cursor-not-allowed" : added ? "bg-success text-white" : "bg-primary text-white hover:bg-primaryDark"
              }`}
            >
              {added ? (<><HiCheck size={18} /> Added to cart</>) : outOfStock ? "Out of stock" : "Add to cart"}
            </button>
          </div>
      ...
      {related.length > 0 && (
        <div className="mt-20">
          <h2 className="font-display text-2xl font-bold mb-6">From the same category</h2>
